...
class TableOfContentsUnstyled extends Component {
  ...
  onChangeCollapse() {
    if (!isCollapseTransitioning) {
      ...
      this.setState((prevState) => ({
        isOpenCollapse: !prevState.isOpenCollapse,
      }));
      setTimeout(() => {
  ...
  renderLevel(sections, useRouterLinks = false, hashPath = [], useHashId = false) {
    ...
    const items = sections.map((section) => {
      ...
      const childHashPath = sectionDepth === 0 && useHashId ? hashPath : [...hashPath, section.name];
      return Object.assign({}, section, {
        heading: !!section.name && children.length > 0,
        content:
          children.length > 0 && this.renderLevel(children, useRouterLinks, childHashPath, sectionDepth === 0),
